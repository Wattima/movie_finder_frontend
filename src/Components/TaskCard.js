  ...
  return (
    <Container>
      <Box
        ...
        <h1>{name}</h1>
        <h3>{description}</h3>
        <h3>Added by: {taskUser.name}</h3>
        <h3>Category: {categories[`${category_id}`]} | Rating: {priority} </h3>
        <Button variant="contained" onClick={handleCompleteToggle}>{is_complete? "Not Watched Yet" : "Watched!"}</Button>
      </Box>
    </Container>
  )
}
