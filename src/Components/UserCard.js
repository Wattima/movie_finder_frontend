...
export default function UserCard({
  ...
  const categories = {1: "Horror", 2: "Drama", 3: "Comedy"}
    ...
    <Container>
      <Box
        ...
        <ul>{tasks.filter((task) => task.user_id == id).map((t) => <li key={t.id}><h2>{t.name}
          ...
          <Button variant={"contained"} onClick={() => handleCompleteClick(t.id)}>{t.is_complete? "Not Watched" : "Watched"}</Button>
        </h2>
          <div>{t.description? t.description : "No desc"} | {categories[t.category_id]} | {t.priority} | {t.is_complete? "Watched :)" : "Not Watched :(" }</div>
          </li>)}
        </ul>
      </Box>
    </Container>
